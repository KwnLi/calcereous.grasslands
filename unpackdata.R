...
for(i in 1:length(sel)){
  ...
  system(paste0("unzip -j ", "/90daydata/geoecoservices/calcereous/grass/*_", sel[i], "_* '*.tif' -d ", grass.folder.i))

  imperv.folder.i <- paste0(datadir,"outdata/imperv/",sel[i])
  dir.create(imperv.folder.i)
  system(paste0("unzip -j ", "/90daydata/geoecoservices/calcereous/imperv/*_", sel[i], "_* '*.tif' -d ", imperv.folder.i))
}
